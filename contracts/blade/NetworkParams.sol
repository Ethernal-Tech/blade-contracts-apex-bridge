...
contract NetworkParams is Ownable2Step, Initializable {
    ...
    event NewValidatorSet(ValidatorSetApex[] addedValidators, address[] removedValidators);
    ...
    function newValidatorSet(
        ...
        for (uint256 i = 0; i < addedValidators.length; i++) {
            require(addedValidators[i].validatorData.length != 0, "VALIDATOR_SET_MUST_CONTAIN_VALIDATOR_DATA");
        }
        emit NewValidatorSet(addedValidators, removedValidators);
    }
}
